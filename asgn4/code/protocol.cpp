...
string to_string(cxi_command command)
{
   switch (command)
   {
      ...
      case cxi_command::RM:
         return "RM";
      case cxi_command::FILEOUT:
         return "FILEOUT";
      case cxi_command::LSOUT:
         return "LSOUT";
      case cxi_command::ACK:
         return "ACK";
      case cxi_command::NAK:
         return "NAK";
      default:
         return "????";
   };
...
void send_packet
   (base_socket &socket, const void *buffer, size_t bufsize)
{
...
in_port_t get_cxi_server_port
   (const vector<string> &args, size_t index)
{
   string port = "-1";
   if (index < args.size())
      port = args[index];
